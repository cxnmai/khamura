...
fn mp4_is_playable_mirrored_and_tracks_wall_clock() {
    ...
    let frame = Arc::new(RgbaImage::from_fn(64, 48, |x, _| {
        if x < 32 { image::Rgba([0, 0, 255, 255]) } else { image::Rgba([255, 0, 0, 255]) }
    }));
    let (recorder, completion) = Recorder::start(output.path(), frame.clone(), true, None, 10).unwrap();
    ...
    let probe = Command::new("ffprobe").args(["-v", "error", "-select_streams", "v:0",
        "-show_entries", "stream=codec_name,width,height,duration", "-of", "default=noprint_wrappers=1"])
    ...
    assert!(metadata.contains("width=64") && metadata.contains("height=48"), "{metadata}");
    let duration: f64 = metadata.lines().find_map(|line| line.strip_prefix("duration=")).unwrap().parse().unwrap();
    assert!((0.5..1.5).contains(&duration), "duration={duration}");
    let decoded = Command::new("ffmpeg").args(["-v", "error", "-i"]).arg(&path)
        .args(["-frames:v", "1", "-f", "rawvideo", "-pix_fmt", "rgb24", "pipe:1"]).output().unwrap();
    assert!(decoded.status.success());
    assert!(decoded.stdout[2] > 200 && decoded.stdout[0] < 40, "Left side should be mirrored blue");
...
fn invalid_microphone_reports_failure_without_publishing() {
    ...
    let (recorder, completion) = Recorder::start(output.path(), frame, false,
        Some("khamura-nonexistent-test-source".into()), 10).unwrap();
    recorder.stop();
