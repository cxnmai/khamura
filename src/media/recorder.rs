...
impl Recorder {
    pub fn start(output: &Path, initial: Arc<RgbaImage>, mirror: bool, microphone: Option<String>, fps: u32)
        -> Result<(Self, async_channel::Receiver<Result<PathBuf, String>>), String>
    {
        ...
        let encoder = Encoder::launch(temporary.path(), initial.width(), initial.height(), fps, mirror, microphone.as_deref())?;
        let recorder = Self { frame: Arc::new(Mutex::new(initial)), stopped: Arc::new(AtomicBool::new(false)) };
        let frame = recorder.frame.clone();
        let stopped = recorder.stopped.clone();
        let (sender, receiver) = async_channel::bounded(1);
        std::thread::spawn(move || {
            let result = record(encoder, frame, stopped, fps).and_then(|()| super::publish(temporary, destination));
    ...
    pub fn update_frame(&self, frame: Arc<RgbaImage>) {
        if let Ok(mut latest) = self.frame.lock() {
            if latest.dimensions() == frame.dimensions() { *latest = frame; }
        }
    }
...
fn record(mut encoder: Encoder, latest: Arc<Mutex<Arc<RgbaImage>>>, stopped: Arc<AtomicBool>, fps: u32) -> Result<(), String> {
    ...
    if flags < 0 || unsafe { libc::fcntl(fd, libc::F_SETFL, flags | libc::O_NONBLOCK) } < 0 {
        return Err(format!("Cannot configure recording pipe: {}", std::io::Error::last_os_error()));
    }
    ...
    loop {
        match encoder.child.try_wait().map_err(|e| e.to_string())? {
            ...
            Some(status) => return Err(encoder.error(&format!("Cannot finalize recording ({status})"))),
            None if Instant::now() > deadline => return Err(encoder.error("Video finalization timed out")),
