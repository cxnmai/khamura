...
impl Encoder {
    pub fn launch(path: &Path, width: u32, height: u32, fps: u32, mirror: bool, microphone: Option<&str>) -> Result<Self, String> {
        let mut command = Command::new("ffmpeg");
        command.args(["-hide_banner", "-loglevel", "error", "-y", "-f", "rawvideo", "-pixel_format", "bgra", "-video_size"]);
        command.arg(format!("{width}x{height}")).args(["-framerate", &fps.to_string(), "-i", "pipe:0"]);
        ...
        command.arg(if mirror { "hflip,pad=ceil(iw/2)*2:ceil(ih/2)*2" } else { "pad=ceil(iw/2)*2:ceil(ih/2)*2" });
        command.args(["-c:v", "libx264", "-preset", "veryfast", "-crf", "20", "-pix_fmt", "yuv420p"]);
        if microphone.is_some() {
            command.args(["-map", "1:a:0", "-c:a", "aac", "-b:a", "128k", "-af", "aresample=async=1", "-shortest"]);
        }
        ...
        let mut child = command.stdin(Stdio::piped()).stdout(Stdio::null()).stderr(Stdio::piped())
            .spawn().map_err(|e| format!("Cannot start FFmpeg (install ffmpeg with H.264 support): {e}"))?;
    ...
    pub fn error(&self, context: &str) -> String {
        ...
        if details.trim().is_empty() { context.to_string() } else { format!("{context}: {}", details.trim()) }
    }
}
