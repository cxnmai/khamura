...
impl Default for CapturePreferences {
    fn default() -> Self {
        Self {
            mode: CameraMode::Photo, timer_seconds: 0, aspect: PhotoAspect::Native,
            grid: false, microphone_on: true, quality: None,
            camera_device: None, microphone_device: None,
        }
    }
}
...
impl CapturePreferences {
    pub fn validate(&self) -> Result<(), String> {
        ...
        if self.quality.is_some_and(|q| q.width == 0 || q.height == 0 || q.fps == 0) {
            return Err("capture.quality dimensions and fps must be positive".into());
        }
        if [&self.camera_device, &self.microphone_device].iter()
...
impl From<&Config> for CaptureSettings {
    fn from(config: &Config) -> Self {
        ...
        Self {
            mode: p.mode, timer_seconds: p.timer_seconds, aspect: p.aspect, grid: p.grid,
            microphone_on: p.microphone_on, quality: p.quality, qualities: Vec::new(),
            camera_device: p.camera_device.clone(), microphone_device: p.microphone_device.clone(),
            busy: false, recording: false, save_error: None,
        }
    }
}
...
impl CaptureSettings {
    ...
    pub fn save(cx: &mut App) {
        ...
        let preferences = CapturePreferences {
            mode: s.mode, timer_seconds: s.timer_seconds, aspect: s.aspect, grid: s.grid,
            microphone_on: s.microphone_on, quality: s.quality,
            camera_device: s.camera_device.clone(), microphone_device: s.microphone_device.clone(),
        };
