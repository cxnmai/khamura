...
pub(crate) fn relocate(config: &Config, home: &Path, target: &Path) -> Result<(), String> {
    ...
    if fs::symlink_metadata(config.path()).is_ok_and(|metadata| metadata.file_type().is_symlink()) {
        return Err(
            "cannot relocate a symlinked configuration; choose a regular file first".into(),
        );
    }
    ...
    let pointer = marker(home);
    if target.file_name() == pointer.file_name()
        && fs::canonicalize(parent).ok() == fs::canonicalize(pointer.parent().unwrap()).ok()
    {
        return Err("configuration cannot replace the location marker".into());
    }
