...
pub(super) fn aspect_label(aspect: PhotoAspect) -> &'static str {
    match aspect {
        PhotoAspect::Native => "Native",
        PhotoAspect::FourThree => "4:3",
        PhotoAspect::SixteenNine => "16:9",
        PhotoAspect::Square => "1:1",
    }
}
pub(super) fn quality_label(quality: Option<VideoQuality>) -> String {
    quality
        .map(|q| format!("{}×{} · {} fps", q.width, q.height, q.fps))
...
pub(super) fn choices(menu: Menu, settings: &CaptureSettings) -> Vec<(String, Choice, bool)> {
    match menu {
        ...
        .collect(),
        Menu::Quality => std::iter::once(None)
            .chain(settings.qualities.iter().copied().map(Some))
            .map(|q| (quality_label(q), Choice::Quality(q), settings.quality == q))
            .collect(),
    }
}
